<!-- Button trigger modal -->
<button type="button" class="btn btn-outline-secondary btn-sm" data-toggle="modal" data-target="#southernMissModalCenter">
  About
</button>

<!-- Modal -->
<div class="modal fade" id="southernMissModalCenter" tabindex="-1" role="dialog" aria-labelledby="southernMissModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header justify-content-center">
        <h5 class="modal-title" id="southernMissModalLongTitle">About the Southern Miss Football Uniform History Table</h5>
      </div>
      <div class="modal-body">
        <div class = "text-left">
            <p>This table contains a record of the uniforms worn in every Southern Miss Golden Eagle football game since 2003.</p>
            <p>I feel very confident in the accuracy of the combinations listed in this table, as there are sufficent photographs from every game
                in the 2003-present timeframe. However, please let me know via the "Report an Issue" button if you see anything off.</p>
        </div>
        <div class="container">
          <img src="https://i.imgur.com/7pI9o0l.png" width="45" alt="" class="d-inline-block align-middle mr-3">
          <button type="button" class="btn btn-md standardBtn" data-dismiss="modal" aria-label="Close">Close</button>
        </div>
      </div>
    </div>
  </div>
</div>

<!-- Report Issue Button -->
<a role="button" class="btn btn-outline-secondary btn-sm" href="report-issue.php">Report an Issue</a>
